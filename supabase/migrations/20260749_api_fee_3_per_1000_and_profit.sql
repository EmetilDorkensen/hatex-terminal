-- ============================================================================
-- Frè API: 3 HTG pou chak 1 000 HTG resevwa (pa 3%)
-- Frè a kredite nan Kès Global (platform_treasury) pou pwofi biznis itilizab
-- ============================================================================
...
BEGIN
  ...
  -- 3 HTG / 1 000 HTG resevwa
  v_api_fee := ROUND((p_amount / 1000.0) * 3, 2);
  ...
  VALUES (
    ...
    'Lavant sou entènèt (net): Kliyan ' || COALESCE(p_client_name, 'Kliyan')
      || ' (Kòmand #' || COALESCE(NULLIF(p_order_id, ''), 'N/A') || ') — frè API 3 HTG/1000: '
      || v_api_fee || ' HTG',
    'success',
    v_transaction_ref || '-M',
    jsonb_build_object(
  ...
  IF v_api_fee > 0 THEN
    ...
    VALUES (
      ...
      'Frè API (3 HTG / 1 000) sou peman Kòmand #' || COALESCE(NULLIF(p_order_id, ''), 'N/A'),
      'success',
      v_transaction_ref || '-F',
      jsonb_build_object(
        'source', 'public_api',
        ...
        'tx_ref', v_transaction_ref,
        'order_id', p_order_id,
        'fee_rule', '3_htg_per_1000'
      )
    );

    -- Monte sou Kès Global (pwofi biznis itilizab)
    BEGIN
