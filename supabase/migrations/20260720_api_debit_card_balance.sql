-- ============================================================================
-- KORIJE: peman API ak kat dwe debite card_balance (pa wallet_balance)
-- ============================================================================
-- Lè yon kliyan peye ak nimewo kat, lajan an soti sou balans KAT la
-- (card_balance), menm jan ak subscribe/checkout. Machann nan toujou
-- resevwa sou wallet_balance li.
-- ============================================================================
...
BEGIN
  ...
  SELECT card_balance, account_status INTO v_client_card_balance, v_client_status
  ...
  IF COALESCE(v_client_card_balance, 0) < p_amount THEN
    RETURN json_build_object(
      'success', false,
      'message', 'Ou pa gen ase lajan sou kat ou a. Balans kat ou se ' || COALESCE(v_client_card_balance, 0) || ' HTG.'
    );
  END IF;
