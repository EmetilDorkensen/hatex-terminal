    ...
    return (
        <div className="min-h-screen bg-[#0a0b14] text-white p-4 font-sans uppercase italic pb-24">
            ...
            <div className="flex gap-2 mb-6 max-w-md mx-auto">
                <button onClick={() => setView('depo')} className={`flex-1 py-4 rounded-2xl text-xs font-black transition-all ${view === 'depo' ? 'bg-red-600' : 'bg-zinc-900 text-zinc-500'}`}>DEPO</button>
                <button onClick={() => setView('retre')} className={`flex-1 py-4 rounded-2xl text-xs font-black transition-all ${view === 'retre' ? 'bg-red-600' : 'bg-zinc-900 text-zinc-500'}`}>RETRÈ</button>
            </div>
            ...
            {loading ? (
                ...
                <div className="space-y-4 max-w-md mx-auto">
                    {(view === 'depo' ? deposits : withdrawals).map((item) => (
                        <div key={item.id} className="bg-zinc-900 p-5 rounded-[2.5rem] border border-white/5 relative shadow-2xl">
                            <div className="flex justify-between mb-2 pr-8">
                                ...
                                <span className={`text-[8px] px-3 py-1 rounded-full font-black ${item.status === 'approved' || item.status === 'completed' ? 'bg-green-500' : item.status === 'rejected' ? 'bg-red-600' : 'bg-yellow-500 text-black'}`}>{item.status}</span>
                            </div>
                            ...
                            {view === 'depo' ? (
                                <div className="space-y-2">
                                    ...
                                    {item.status === 'pending' && (
                                        <button disabled={processingId === item.id} onClick={() => anileDepo(item)} className="w-full bg-red-600/10 text-red-600 border border-red-600/20 py-3 rounded-xl text-[10px] font-black">ANILE DEPO</button>
                                    )}
                                </div>
                            ) : (
                                <div className="space-y-3">
                                    <div className="bg-black/40 p-4 rounded-2xl border border-white/5">
                                        ...
                                        <p className="text-xs font-black">{item.phone || item.account_number}</p>
                                    </div>
                                    {item.status === 'pending' && (
                                        <button disabled={processingId === item.id} onClick={() => finPeyeRetre(item)} className="w-full bg-white text-black py-4 rounded-xl text-[10px] font-black uppercase">
                                            {processingId === item.id ? 'Loading...' : 'Konfime Pèman'}
                                        </button>
                                    )}
                                </div>
                            )}
