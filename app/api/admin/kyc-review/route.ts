...
import { hasValidAdminGate } from '@/lib/admin/auth';
...
export async function POST(request: Request) {
  ...
  if (action === 'approved') {
    ...
    // Kreye kat otomatikman MEN bloke (kle) jiskaske dezyèm 525 HTG peye
    await provisionCardForUser(admin, userId, { activate: false });
    ...
    if (isAdmin) {
      await logAdminAction(admin, {
        ...
        targetType: 'profile',
        targetId: userId,
        details: { locked_until_unlock_fee: true },
        ip,
      });
    }
