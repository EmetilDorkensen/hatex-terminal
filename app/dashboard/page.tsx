...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0b14] text-white font-sans relative flex flex-col italic overflow-x-hidden pb-24">
      ...
      <div className="w-full max-w-md mx-auto p-5 pb-0">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-1 bg-zinc-900/50 rounded-full border border-zinc-800 flex items-center justify-center active:scale-90 transition-all shrink-0"
            >
              <span className="text-[9px] font-black uppercase text-zinc-400">
                {showNumbers ? "KACHE" : "WÈ"}
              </span>
            </button>
          </div>
        </div>
                    ...
                    recentTransactions.map((t) => (
                        <div key={t.id} className="bg-zinc-900/40 border border-white/5 p-4 rounded-[2rem] backdrop-blur-md active:scale-95 transition-all">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-zinc-800/80 border border-white/5 font-mono text-[9px] font-black text-zinc-500">
                                        {t.type === 'DEPOSIT' ? 'DEP' : 
                                         t.type === 'WITHDRAWAL' ? 'RET' : 
                                         t.type === 'P2P' ? 'TRF' : 
                                         t.type === 'CARD_RECHARGE' ? 'KAT' : 'LÒT'}
                                    </div>
                                    <div>
        ...
        {/* 6. NAVIGASYON ANBA (FOOTER TEXT ONLY) */}
        <div className="fixed bottom-4 left-4 right-4 max-w-md mx-auto bg-zinc-900/95 backdrop-blur-xl border border-white/10 h-16 rounded-[1rem] flex justify-between items-center px-6 z-50 shadow-2xl">
          <button onClick={() => window.scrollTo(0, 0)} className="flex flex-col items-center gap-0.5">
              <span className="text-[10px] font-black uppercase text-red-600 tracking-widest">Akey</span>
              <div className="w-3 h-0.5 bg-red-600 rounded-full"></div>
          </button>
          
          <button onClick={() => router.push('/kat')} className="flex flex-col items-center opacity-60 hover:opacity-100 transition-opacity">
            <span className="text-[10px] font-black uppercase text-white tracking-widest">Kat</span>
          </button>

          <button onClick={() => router.push('/terminal')} className="flex flex-col items-center justify-center bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 opacity-80 hover:opacity-100 active:scale-95 transition-all">
            <span className="text-[10px] font-black uppercase text-white tracking-widest">Terminal</span>
          </button>
          
          <button onClick={() => router.push('/transactions')} className="flex flex-col items-center opacity-60 hover:opacity-100 transition-opacity">
            <span className="text-[10px] font-black uppercase text-white tracking-widest">Istorik</span>
          </button>
          
          <button onClick={async () => { await supabase.auth.signOut(); window.location.href = '/login'; }} className="flex flex-col items-center opacity-60 hover:opacity-100 transition-opacity">
            <span className="text-[10px] font-black uppercase text-red-500 tracking-widest">Soti</span>
          </button>
        </div>
      </div>
