...
import { 
  ...
  ArrowLeft, Building2, Briefcase, UploadCloud, CheckSquare, ChevronRight, Copy, XCircle, CreditCard
} from 'lucide-react';

// Diferan etap pou paj la
...
export default function AgentPortal() {
  ...
  const [paymentPlan, setPaymentPlan] = useState<'full' | 'installments' | null>(null);
  ...
  // ==========================================
  // VALIDE DOKIMAN YO POU PASE NAN CHWA PÈMAN AN
  // ==========================================
  const handleDocsNext = () => {
    if (!idDoc || !addressDoc || !locationPhoto) {
      return alert("Pyès idantite, prèv adrès ak foto lokal la obligatwa.");
    }
    if (selectedTier === 'premium' && (!patenteDoc || !cifDoc)) {
  ...
  // ==========================================
  // SOUMÈT APLIKASYON AN BAY ADMIN AN
  // ==========================================
  const submitApplication = async () => {
    if (!paymentPlan) return alert("Tanpri chwazi kòman w ap peye anvan ou soumèt.");

    setActionLoading(true);
    try {
      // 1. Fonksyon pou telechaje dosye nan Supabase Storage
      const uploadFile = async (file: File, type: string) => {
        ...
        if (error) {
           ...
           return "url_pwovizwa_pou_dokiman"; // Fallback si bokit la poko byen konfigire
        }
      ...
      // 3. Ekri nan tab aplikasyon an (pou Admin an ka verifye)
      await supabase.from('agent_applications').insert([{
        ...
        cif_url: cifUrl,
        metadata: { payment_plan: paymentPlan } // Nou anrejistre chwa peman an tou
      }]);

      // 4. Mete estati pwofil la sou 'pending'
      await supabase.from('profiles').update({ 
        agent_status: 'pending', 
        agent_tier: selectedTier 
      }).eq('id', profile.id);

      // Rafrechi paj la pou l tonbe sou 'pending' (1 a 3 jou)
      fetchAgentData();
  ...
  // ==========================================
  // REKÒMANSE APLIKASYON (Lè l rejected)
  // ==========================================
  ...
  // ==========================================
  // FÈ VÈSMAN GARANTI (Lè l fin Apwouve nan Dashboard la)
  // ==========================================
  const makeInstallmentPayment = async () => {
    ...
    try {
      ...
      const newCapacity = newGuaranteePaid; // Kapasite kont la se kantite li peye a
      ...
      await supabase.from('transactions').insert([{
        ...
        amount: -amount,
        status: 'success',
        description: `Vèsman pou kapasite Ajan ${profile.agent_tier.toUpperCase()}`
      }]);

      alert(`Vèsman ${amount.toLocaleString()} HTG pase ak siksè! Kapasite Ajan w lan ogmante.`);
  ...
  // ==========================================
  if (appStep === 'loading') {
    return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><Loader2 className="animate-spin text-indigo-600 w-12 h-12" /></div>;
  }

  // ==========================================
  // VIEW: KYC DENIED (Moun nan poko verifye)
  // ==========================================
      ...
      <div className="min-h-screen bg-slate-50 text-slate-900 flex items-center justify-center p-6 font-sans">
        <div className="bg-white border border-gray-200 p-8 rounded-3xl shadow-sm text-center max-w-md w-full">
          ...
          <p className="text-sm text-slate-500 mb-8 leading-relaxed">
            Pou garanti sekirite platfòm nan, ou dwe pase etap verifikasyon idantite a (KYC) anvan ou ka aplike pou vin yon ajan Hatexcard.
          </p>
  ...
  // ==========================================
  // VIEW: REJECTED (Admin rejte dokiman yo)
  // ==========================================
  if (appStep === 'rejected') {
    return (
      <div className="min-h-screen bg-slate-50 text-slate-900 flex items-center justify-center p-6 font-sans">
        <div className="bg-white border border-rose-200 p-8 rounded-3xl shadow-sm text-center max-w-md w-full">
          <XCircle size={60} className="text-rose-500 mx-auto mb-6" />
          <h2 className="text-xl font-bold tracking-tight text-slate-900 mb-3">Dokiman ou yo gen pwoblèm</h2>
          <p className="text-sm text-slate-500 mb-8 leading-relaxed">
            Apre verifikasyon, ekip la pa t ka apwouve aplikasyon w lan. Sa ka rive si dokiman yo pa klè, yo ekspire, oswa enfòmasyon yo pa koresponn.
          </p>
        ...
        <div className="bg-white border border-indigo-100 p-8 sm:p-10 rounded-3xl shadow-sm text-center max-w-lg w-full">
          ...
          <div className="bg-slate-50 border border-gray-100 p-5 rounded-2xl mb-8">
            <p className="text-sm text-slate-600 leading-relaxed font-medium">
              Dokiman ou yo byen rive nan baz done a! Ekip Admin nou an ap verifye tout pyès idantite ak prèv ou soumèt yo.
            </p>
            <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider mt-4 bg-indigo-50 py-2 rounded-lg">
              Tan atant: 1 a 3 Jou Ouvrab
            </p>
          </div>
          <button onClick={() => router.push('/dashboard')} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 py-4 rounded-xl font-bold text-xs uppercase tracking-wider transition-all shadow-sm">
  ...
  // ==========================================
  // VIEW: ETAP 1 - CHWAZI NIVO AJAN AN
  // ==========================================
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white border border-gray-200 p-8 rounded-3xl shadow-sm">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-2">Pyès Idantite Valid (Mèt/Anplwaye) *</label>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-2">Foto Kote W Vle Bay Sèvis La (Devan & Kote) *</label>
            ...
            <button onClick={handleDocsNext} className="w-full mt-10 bg-slate-900 hover:bg-slate-800 text-white py-4 rounded-xl font-bold uppercase tracking-wider text-xs transition-all shadow-sm flex justify-center items-center gap-2">
  ...
  // ==========================================
  // RENDER: ETAP 3 - CHWAZI FASON POU PEYE GARANTI A
  // ==========================================
  if (appStep === 'payment_plan') {
    ...
    return (
      <div className="min-h-screen bg-slate-50 text-slate-900 p-6 font-sans pb-24">
        <div className="max-w-2xl mx-auto">
          <button onClick={() => setAppStep('upload_docs')} disabled={actionLoading} className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 font-bold text-xs uppercase tracking-wider mb-6 transition-colors disabled:opacity-50"><ArrowLeft size={16} /> Tounen</button>
          
          <div className="bg-white border border-gray-200 p-8 rounded-3xl shadow-sm">
            <h2 className="text-2xl font-bold tracking-tight mb-2">Fason Peman Garanti a</h2>
            <p className="text-sm text-slate-500 mb-8 leading-relaxed">
              Kantite lajan ou peye kòm garanti a se limit maksimòm (kapasite) kòb ou ka gen sou kont ajan w lan pou w travay la. Kòman w ta renmen peye {requiredAmount.toLocaleString()} HTG sa a?
            </p>
            ...
            <div className="space-y-4 mb-10">
              ...
              <div 
                ...
                className={`p-6 rounded-2xl border-2 cursor-pointer transition-all ${paymentPlan === 'full' ? 'border-indigo-600 bg-indigo-50/50' : 'border-gray-200 hover:border-indigo-300'}`}
              >
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-bold text-slate-900 flex items-center gap-2">
                    {paymentPlan === 'full' ? <CheckCircle2 className="text-indigo-600" size={20}/> : <div className="w-5 h-5 rounded-full border-2 border-gray-300"></div>}
                    Peye tout yon sèl kou
                  </h3>
                  <span className="text-sm font-bold text-indigo-600">{requiredAmount.toLocaleString()} HTG</span>
                </div>
                <p className="text-xs text-slate-500 pl-7">W ap gen aksè ak tout kapasite maksimòm ajan w lan depi premye jou admin nan apwouve w la.</p>
              </div>

              {/* Option 2: Installments */}
              <div 
                onClick={() => setPaymentPlan('installments')}
                className={`p-6 rounded-2xl border-2 cursor-pointer transition-all ${paymentPlan === 'installments' ? 'border-indigo-600 bg-indigo-50/50' : 'border-gray-200 hover:border-indigo-300'}`}
              >
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-bold text-slate-900 flex items-center gap-2">
                    {paymentPlan === 'installments' ? <CheckCircle2 className="text-indigo-600" size={20}/> : <div className="w-5 h-5 rounded-full border-2 border-gray-300"></div>}
                    Peye nan 3 Vèsman
                  </h3>
                </div>
                <p className="text-xs text-slate-500 pl-7 leading-relaxed">
                  Ou ka peye pa ti moso. Kapasite kont ou an ap elaji ofiramezi ou peye. (Egz: Si w peye 10,000 HTG jodi a, w ap ka fonksyone sèlman ak yon limit 10,000 HTG sou kont ajan an).
                </p>
              </div>
            </div>

            <button onClick={submitApplication} disabled={actionLoading || !paymentPlan} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-xl font-bold uppercase tracking-wider text-xs transition-all shadow-sm flex justify-center items-center gap-2 disabled:opacity-50 disabled:bg-slate-300">
              {actionLoading ? <Loader2 className="animate-spin w-4 h-4" /> : 'Soumèt Aplikasyon an pou Verifikasyon'}
            </button>
            <p className="text-[10px] text-center font-bold text-slate-400 mt-4 uppercase tracking-wider">Apre w fin soumèt, w ap tann 1 a 3 jou ouvrab.</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
             ...
             <div className="bg-slate-50 border border-gray-200 px-5 py-3 rounded-2xl flex items-center gap-4 w-full md:w-auto">
               ...
               <button onClick={() => { navigator.clipboard.writeText(profile?.agent_code); alert('Kòd la kopye!'); }} className="p-2 bg-white rounded-lg border border-gray-200 text-slate-500 hover:text-indigo-600 shadow-sm transition-colors">
                  <Copy size={16} />
               </button>
             </div>
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-6">
             {/* KÒLÒN GOCH (Balans, Kapasite & Fòm Depo) */}
                ...
                <div className="bg-indigo-600 text-white p-8 rounded-3xl shadow-md relative overflow-hidden">
                   ...
                   <div className="relative z-10">
                     ...
                     <div className="bg-indigo-500/50 border border-indigo-400/50 p-4 rounded-xl mt-4">
                       ...
                       {/* Si l gen vèsman ki rete pou l fè sou garanti a */}
                         ...
                         <div className="mt-4 pt-4 border-t border-indigo-400/30">
                            ...
                            <div className="flex gap-2">
                              <input 
                                ...
                                className="w-full bg-indigo-900/30 border border-indigo-400/30 py-2.5 px-3 rounded-lg text-sm font-bold outline-none text-white placeholder:text-indigo-300/50" 
                              />
                ...
                <div className="bg-white border border-gray-200 p-6 sm:p-8 rounded-3xl shadow-sm">
                   <div className="flex items-center gap-3 mb-6">
                     ...
                     <div>
                       ...
                       <p className="text-xs font-medium text-slate-500">Transfere soti nan balans ajan w lan.</p>
                     </div>
                   </div>
                   ...
                   <form onSubmit={handleAgentDeposit} className="space-y-5">
                     ...
                     <button 
                       ...
                       className="w-full bg-slate-900 hover:bg-slate-800 text-white py-4 rounded-xl font-bold uppercase tracking-wider text-xs transition-all shadow-sm flex items-center justify-center gap-2 disabled:opacity-50"
                     >
                       {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Voye Depo a bay Kliyan an'}
                     </button>
                     <p className="text-[9px] text-center font-bold text-slate-400 uppercase tracking-widest mt-3">Depo kliyan yo 100% gratis san frè.</p>
                   </form>
                </div>

                {/* NÒT POU RETRÈ */}
                <div className="bg-amber-50 border border-amber-200 p-6 rounded-3xl shadow-sm text-amber-800">
                  <h4 className="font-bold mb-2 flex items-center gap-2"><ArrowUpFromLine size={18} /> Kòman Retrè yo fèt?</h4>
                  <p className="text-sm font-medium leading-relaxed">
                    Lè yon kliyan bezwen fè retrè nan men w, se <strong>li menm</strong> ki dwe inisye tranzaksyon an sou aplikasyon l lan, epi antre kòd 8-chif ou an (<span className="font-mono font-bold bg-amber-100 px-1 rounded">{profile?.agent_code}</span>). Kòb la ap ajoute otomatikman sou balans Ajan w lan imedyatman epi w ap ba l lajan l nan men.
                  </p>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden h-full">
                   <div className="p-6 md:p-8 border-b border-gray-100 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-medium mt-0.5">Tout depo ak retrè ou jere yo</p>
                      </div>
                   ...
                   <div className="p-4 sm:p-6 md:p-8">
                      {agentTransactions.length === 0 ? (
                        <div className="text-center py-24 bg-slate-50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <div className="space-y-3">
                           {agentTransactions.map((tx) => (
                             <div key={tx.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 sm:p-5 rounded-2xl border border-gray-100 bg-white hover:bg-slate-50 hover:shadow-sm hover:border-indigo-100 transition-all shadow-sm">
                               <div className="flex items-center gap-4">
                                 <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white shrink-0 shadow-sm ${tx.type === 'AGENT_WITHDRAWAL' ? 'bg-emerald-500' : 'bg-rose-500'}`}>
                                   {tx.type === 'AGENT_WITHDRAWAL' ? <ArrowUpFromLine size={20} /> : <ArrowDownToLine size={20} />}
                                 </div>
                                 <div>
                                   <p className="text-sm font-bold text-slate-900">{tx.metadata?.client_email || 'Kliyan Hatex'}</p>
                                   <div className="flex items-center gap-2 mt-1">
                                     <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${tx.type === 'AGENT_WITHDRAWAL' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-rose-50 text-rose-700 border border-rose-100'}`}>
                               ...
                               <div className="text-right border-t sm:border-none border-gray-100 pt-3 sm:pt-0">
                                 <p className={`text-base font-bold ${tx.type === 'AGENT_WITHDRAWAL' ? 'text-emerald-600' : 'text-slate-900'}`}>
                                   {tx.type === 'AGENT_WITHDRAWAL' ? '+' : ''}{Math.abs(tx.amount).toLocaleString()} <span className="text-xs text-slate-500">HTG</span>
                                 </p>
                                 <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md mt-1.5 inline-block border border-emerald-100">Reyisi</span>
                               </div>
                             </div>
                           ))}
                        </div>
  ...
  // Fallback sekirite (Pa ta dwe janm afiche)
  return null;
}